...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (q_input, k_input, v3_input) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)

        q_input = q_input.float().cuda(non_blocking=True)
        k_input = k_input.float().cuda(non_blocking=True)
        v3_input = v3_input.float().cuda(non_blocking=True)
        
        # compute output
        # ===== Pass A: 原两视图 =====
        model.enqueue_enabled = True 
        output1_a, output2_a, output3_a, output4_a, target1_a, target2_a, target3_a, target4_a = model(q_input, k_input)
        batch_size = output2_a.size(0) 
        loss_a = (criterion(output1_a, target1_a)
            + criterion(output2_a, target2_a)
            + criterion(output3_a, target3_a)
            + criterion(output4_a, target4_a))
        # ===== Pass B: 第三视图参与（复用同一对编码器；固定 k_input 做“teacher”更稳）=====
        model.enqueue_enabled = False 
        output1_b, output2_b, output3_b, output4_b, target1_b, target2_b, target3_b, target4_b = model(v3_input, k_input)

        loss_b = (criterion(output1_b, target1_b)
                + criterion(output2_b, target2_b)
                + criterion(output3_b, target3_b)
                + criterion(output4_b, target4_b))

       # ===== 聚合：两次 loss 取均值，保持标度稳定 =====
        loss = 0.5 * (loss_a + loss_b)

        losses.update(loss.item(), batch_size)
 
       # 记录指标（保持与原来一致：用 output2 的 top1）
        losses.update(loss.item(), batch_size)
        acc1, _ = accuracy(output2_a, target2_a, topk=(1, 5))  # 也可换成 output2_b，看你想监控哪一路
